...
class WooCommerceContentGenerator extends AbstractContentGenerator
{
    ...
    {
        if (!empty($options['postTemplate']) && is_array($options['postTemplate'])) {
            $templateGenerator = new PostTemplateBlockGenerator($options['postTemplate'], $options);
            if ($layout = $this->getLayout()) {
                $templateGenerator->setLayout($layout);
            }
            return $templateGenerator->generate($query, $options);
        }
            ...
            while ($query->have_posts()) {
                ...
                <li class="<?php echo esc_attr($product_classes); ?>" data-wp-interactive="woocommerce/product-collection" data-wp-context="<?php echo esc_attr(json_encode(['productId' => $product_id])); ?>" data-wp-key="product-item-<?php echo esc_attr($product_id); ?>">
                    ...
                    <div data-block-name="woocommerce/product-image" data-image-sizing="thumbnail" data-is-descendent-of-query-loop="true" data-show-sale-badge="<?php echo $showSaleBadge ? 'true' : 'false'; ?>" class="wc-block-components-product-image wc-block-grid__product-image wp-block-woocommerce-product-image">
                        <a href="<?php echo esc_url(get_permalink()); ?>" data-wp-on--click="woocommerce/product-collection::actions.viewProduct">
                    ...
                    <?php if ($showTitle): ?>
                    <h2 class="has-text-align-center wp-block-post-title has-medium-font-size" style="line-height:1.4;margin-bottom:0.75rem;margin-top:0;">
                    ...
                    <?php $notice_id = 'wc-add-to-cart-notice-' . $product_id . '-' . wp_unique_id(); ?>
                    <div data-block-name="woocommerce/product-button" data-font-size="small" data-is-descendent-of-query-loop="true" data-text-align="center" class="wp-block-button wc-block-components-product-button align-center wp-block-woocommerce-product-button has-small-font-size" data-wp-interactive="woocommerce/product-button" data-wp-init="actions.refreshCartItems" data-wp-context="<?php echo esc_attr(json_encode([
                        ...
                        <button class="wp-block-button__link wp-element-button wc-block-components-product-button__button add_to_cart_button ajax_add_to_cart product_type_<?php echo esc_attr($product_type); ?> has-font-size has-small-font-size" type="button" data-product_id="<?php echo esc_attr($product_id); ?>" data-product_sku="<?php echo esc_attr($product->get_sku()); ?>" aria-label="<?php echo esc_attr(sprintf(__('Thêm vào giỏ hàng: "%s"', 'woocommerce'), get_the_title())); ?>" data-wp-on--click="actions.addCartItem">
                            <span data-wp-class--wc-block-slide-in="state.slideInAnimation" data-wp-class--wc-block-slide-out="state.slideOutAnimation" data-wp-on--animationend="actions.handleAnimationEnd" data-wp-watch="callbacks.startAnimation" data-wp-run="callbacks.syncTempQuantityOnLoad" data-wp-on--click="actions.handlePressedState">
                                <?php echo $is_in_cart ? sprintf(__('Có %d trong giỏ hàng', 'woocommerce'), $cart_quantity) : __('Thêm vào giỏ hàng', 'woocommerce'); ?>
                            </span>
                        </button>
                        <?php else: ?>
                        <a class="wp-block-button__link wp-element-button wc-block-components-product-button__button product_type_<?php echo esc_attr($product_type); ?> has-font-size has-small-font-size" href="<?php echo esc_url(get_permalink()); ?>" rel="nofollow" data-product_id="<?php echo esc_attr($product_id); ?>" data-product_sku="<?php echo esc_attr($product->get_sku()); ?>" aria-label="<?php echo esc_attr(sprintf(__('Đọc thêm về "%s"', 'woocommerce'), get_the_title())); ?>" data-wp-on--click="woocommerce/product-collection::actions.viewProduct">
   ...
   public function getName(): string
    ...
    protected function getProductClasses($product): string
    {
        ...
        $classes[] = $product->is_in_stock() ? 'instock' : 'outofstock';
